...
import { ArrowLeft, Save, Briefcase, DollarSign, Sliders, ShieldAlert, AlertCircle } from 'lucide-react';
    ...
    async function loadJob() {
      try {
        ...
        if (job) {
          ...
          setDistance(job.distance_from_21237);
          setAppLink(job.application_link || '');
          setPayMin(job.pay_min);
          setPayMax(job.pay_max);
          setPayType(job.pay_type);
          setJobType(job.job_type);
          setVehicleType(job.vehicle_type);
          setCdlRequired(job.cdl_required);
          setStabilityScore(job.stability_score);
          setIncomeScore(job.income_potential_score);
          setBeginnerScore(job.beginner_friendly_score);
          setQuickApplyScore(job.quick_apply_score);
          setCertDifficulty(job.certification_difficulty_score);
          setBackgroundCheck(job.background_check_required);
          setDrugTest(job.drug_test_required);
          setMvrCheck(job.mvr_check_required);
          setInsurance(job.insurance_requirements || '');
          setExperience(job.experience_requirements || '');
          setCertInput(job.required_certifications.join(', '));
          setNotes(job.notes || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updates: Partial<Job> = {
        ...
        application_link: appLink || undefined,
  ...
  if (fetching) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-sm text-slate-400">Loading Job Specification for Edit...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        <h2 className="text-xl font-bold text-slate-100">Edit Job Entry</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="glass-panel p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Tracker Status</label>
              <select
                value={status}
